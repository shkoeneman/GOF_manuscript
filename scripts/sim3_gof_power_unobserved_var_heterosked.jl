...
plot_data = @distributed (append!) for kk = 1:length(ns)
    ...
    for jj = 1:num_iters
        ...
        for ii = 1:length(dataset.y)
            dataset.y[ii] = intercept + b1*dataset.x[ii] + b2*dataset.z[ii] + rand(Normal(0,true_sig + 5*rand()),1)[1]
        end
...
#Save
CSV.write("C:/Users/shkan/OneDrive/Documents/Research/Thesis_GOF_Test_Paper/GOF_manuscript/data/sim3_data.csv",plot_data)
