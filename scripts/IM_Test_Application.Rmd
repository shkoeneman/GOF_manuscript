...
full_data <- read.csv("../data/death_data_application.csv")

#Largest candidate model formula
largest_form <- formula(TARGET_deathRate ~ 1 + incidenceRate + povertyPercent + medIncome + MedianAge
         )

#Fit largest candidate model
model <- lm(largest_form, data = full_data)
y <- matrix(model.response(model.frame(model)), ncol = 1)
n <- nrow(y)
X <- model.matrix(model)
p <- ncol(X)
b <- matrix(coef(model), nrow = p, ncol = 1)
sighat2 <- as.numeric(t(y-(X%*%b)) %*% (y-(X%*%b))*(1/n))
boot_test <- DBModelSelect::BootGOFTestLM(model, data = full_data, return_dist = TRUE)

Classic <- as.numeric(1-pchisq(IMTestStat(b, n, y, X, p, sighat2)$TestStat,df = sum(1:(p+1))))
OPG <- as.numeric(1-pchisq(OPGTest(b, n, y, X, p, sighat2)$TestStat,df = sum(1:(p+1))))
BootV <- as.numeric(BootIMTest(b, n, y, X, p, sighat2)$pval)
BootGOF <- boot_test$boot_int
BreuschPagan <- as.numeric( (lmtest::bptest(model))$p.value)
White <- as.numeric( (whitestrap::white_test(model))$p_value)
...
#Try to narrow in on p-value for BootGOF
level <- .04
unname(quantile(boot_test$boot_dist, c((1 - level)/2, (1 - (1 - level)/2))))
level <- .05
unname(quantile(boot_test$boot_dist, c((1 - level)/2, (1 - (1 - level)/2))))
