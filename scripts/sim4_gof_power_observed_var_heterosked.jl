...
#Set random seed
@everywhere Random.seed!(myid()+1000)
...
plot_data = @distributed (append!) for kk = 1:length(ns)
    ...
    for jj = 1:num_iters
        ...
        for ii = 1:length(dataset.y)
            dataset.y[ii] = intercept + b1*dataset.x[ii] + b2*dataset.z[ii] + rand(Normal(0,true_sig + 0.5*dataset.z[ii]),1)[1]
        end
...
#Save
CSV.write("C:/Users/shkan/OneDrive/Documents/Research/Thesis_GOF_Test_Paper/GOF_manuscript/data/sim4_data.csv",plot_data)
