...
\section{Background Derivations and Test Formulation}
...
\subsection{Goodness-of-Fit Term Variance}
		
		We will now explore the variance of the log-likelihood goodness-of-fit term present in likelihood-based information criteria. We will assume a
		...
		By its nature, the bootstrap test is not designed to produce p-values. However, one may infer a p-value based on a comparison of bootstrap intervals of varying coverage probabilities. For $\alpha_1 < \alpha_2$, if a 
		$100*(1-\alpha_1)$\% bootstrap confidence interval contains the null value $2n$, but a $100*(1-\alpha_2)$\% bootstrap confidence interval does not contain $2n$, one can assume that 
		the analgous p-value for the test would be between $\alpha_1$ and $\alpha_2$.

		An implementation of this bootstrap test was developed in the R package \texttt{DBModelSelect} \citep{Koeneman}, available for download via CRAN and Github.
...
\includegraphics[width=4.5in]{sim3_residual_plot.pdf}\par
...
We now apply the developed test, along with each test examined in the simulation section, to a real dataset as a demonstration. Say an investigator wishes to explore the relationship between cancer-related mortality in
the United States at the county level and various other variables. The primary outcome of interest will be cancer mortalities per 100,000 county residents, with data being collected for 100 U.S. counties. Additionally, the study has access to the
following covariates for each county: cancer diagnoses per 100,000 county residents, median household income in the county in tens of thousands of dollars, median age of county residents, and 
percent of households in the county that are in poverty.

We wish to build a statistical model to explore which covariates are related to mortality at the county level, and would like to assess whether a normal linear regression model is an 
appropriate choice. Initially, a normal linear regression model with cancer mortality as the outcome, and with an intercept term in addition to all
other available variables as covariates, will be fit and considered. Then, each test assessed in the simulation section will be applied to the fitted model at the
$\alpha = 0.05$ level. The code and data used to perform this demonstration can once again be found at \url{https://github.com/shkoeneman/GOF_manuscript/tree/main/scripts}.

We first consider the result of the bootstrap goodness-of-fit test developed above. The 95\% bootstrap interval generated in this analysis was found to be 
$\left[ 133.87, 296.32 \right]$, which does contain the null value of $2n = 200$, and thus we would fail to reject our null hypothesis that the fitted model shows
adequate fit. Using the bootstrap distribution from the test to construct more intervals, we find that a 5\% interval
($\left[ 195.56, 200.15 \right]$) barely contains $2n$, while a 4\% interval ($\left[ 196.17, 199.54 \right]$) does not contain $2n$. Thus,
we can infer that the p-value for our test lies between $0.96$ and $0.95$.

With this information in hand, we now refer to the following table which compares p-values generated by each test from the fitted model.

\begin{table}[h]
	\caption{Application Test Results - P-Values}\label{apptab}%
	%\resizebox{1\textwidth}{!}{%
	\begin{tabular}{ c|c|c|c|c|c|c}
	\toprule
	Boot GOF & White & Breusch-Pagan & Classical IM & Regression IM & Boot IM \\
	\midrule
	$0.95<p<0.96$ & 0.1686 & 0.1486 & 0.9998 & 0.0001 & 0.9987 \\
	\botrule
	\end{tabular}
	%}
...
From the results, we observe that only the regression information matrix test would reject the null hypothesis, which is not surprising given its
anti-conservative nature at lower sample sizes. The classical and bootstrap information matrix tests do not find enough evidence to reject the null
hypothesis, with p-values similar to that of the bootstrap goodness-of-fit test. The White and Breusch-Pagan tests do not reject the null hypothesis,
but show markedly lower p-values, indicating that there may be traces of heteroskedasticity present. In summary, further investigation of this modeling
framework may be needed, but we do not find strong evidence that a normal linear regression model would be inappropriate in this example.
